...
import com.github.armedis.redis.command.generic.RedisTtlRequest;
...
/**
 * Redis ttl http request endpoint service.
 * @author krisjey
 *
 */
@Component
public class RedisTtlService extends BaseService {
    private final Logger logger = LoggerFactory.getLogger(this.getClass());

    private static final String REDIS_COMMAND = "ttl";
    ...
    /**
     * Process ttl command request by x-www-form-urlencoded without redis key at URL.
     * @param redisRequest
     * @return HttpResponse
     */
    @Get
    ...
    public HttpResponse urlencodedWithoutKey(RedisTtlRequest redisRequest) {
    ...
    /**
     * Process ttl command request by x-www-form-urlencoded with redis key at URL.
     * @param redisRequest
     * @return
     */
    @Get
    ...
    @Consumes(MediaTypeNames.FORM_DATA)
    public HttpResponse urlencodedWithKey(RedisTtlRequest redisRequest) {
        logger.info("Text request " + REDIS_COMMAND + " command without key at URL " + redisRequest.toString());
    ...
    /**
     * Process ttl command request by application json without redis key at URL.
     * 
    ...
    /**
     * Process ttl command request by application json with redis key at URL.
     * 
     * When request body is absent then JacksonRequestConverterFunction not working. <br/>
     * So, just use AggregatedHttpRequest.contentUtf8() method and convert to JsonNode.
     * @param httpRequest
